...
def process_story(json_path, image_pipe, tts_model):
    ...
    desc_parts = [f"The main character is {name}, a {species}."]
    if body_features: desc_parts.append(f"Physical features: {body_features}.")
    if outfit or accessories:
        wearings = [w for w in [outfit, accessories] if w] 
        desc_parts.append(f"Wearing: {' and '.join(wearings)}.")
    if base_clothing: desc_parts.append(f"Clothing state: {base_clothing}.")
        
    character_card = " ".join(desc_parts)
    print(f"  [系統] 全局角色卡已載入: {character_card}")
